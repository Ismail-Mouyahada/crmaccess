                                                    ...
                                                    <div class="formgrid grid">
                                                        ...
                                                        <div class="col-12 md:col-9">
                                                            <api-data-source :enable-cache="true"   api-path="components_data/societe_id_option_list" >
                                                                <template v-slot="req">
                                                                    <Dropdown @update:model-value="(val)=>societeIdAutofill(val)" class="w-full" :class="getErrorClass('societe_id')"   :loading="req.loading"   optionLabel="label" optionValue="value" ref="ctrlsociete_id"  v-model="formData.societe_id" :options="req.response" label="Societe"  placeholder="Sélectionnez une valeur" >
                                                                    </Dropdown> 
...
<script setup>
	...
	const props = defineProps({
		...
		routeName : {
			type : String,
			default : 'compteurselecadd',
		},
		apiPath : {
			type : String,
			default : 'compteurselec/add',
		},
		submitButtonLabel: {
			type: String,
			default: "Soumettre",
	...
	//auto fill other inputs on societe_id change
	async function societeIdAutofill(val){
		val = encodeURIComponent(val);
		const resp = await ApiService.get(`components_data/compteurselec_societe_id_autofill?value=${val}`);
		const data = resp.data[0];
		if(data){
			formData.societe_id = data.id;
		}
	}
