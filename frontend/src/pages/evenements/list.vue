                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/evenements'}" :model="pageBreadCrumb" />
                            </template>
                                    ...
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="id" header="Id" >
                                            <template #body="{ data, index }">
                                                <router-link :to="`/evenements/view/${data.id}`">
                                                    {{ data.id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="type_evenement" header="Type Evenement" >
                                            <template #body="{ data, index }">
                                                {{ data.type_evenement }}
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="date_rendez_vous" header="Date Rendez Vous" >
                                            <template #body="{ data, index }">
                                                {{ data.date_rendez_vous }}
                                            </template>
                                        </Column>
                                        <Column  field="date_creation" header="Date Creation" >
                                            <template #body="{ data, index }">
                                                {{ data.date_creation }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'evenementslist',
		},
		apiPath: {
			type: String,
			default: 'evenements/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/evenements/view/${data.id}`,
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			to: `/evenements/edit/${data.id}`,
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
